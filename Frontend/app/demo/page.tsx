...
export default function Home() {
  ...
  const aggressiveUpdateIntervalRef = useRef<NodeJS.Timeout | null>(null);
  ...
  const [isTopNavPanelExpanded, setIsTopNavPanelExpanded] = useState(true);
  const [isMapPickerActive, setIsMapPickerActive] = useState<'start' | 'destination' | null>(null);
  ...
  const handleStartSimulation = (
    ...
    // Don't automatically freeze map view - respect user's current setting
    // setIsMapViewFreezed(true); // REMOVED: Let user control this
  ...
  useEffect(() => {
    return () => {
      ...
      if (aggressiveUpdateIntervalRef.current) {
        clearInterval(aggressiveUpdateIntervalRef.current);
      }
    };
  }, [animationInterval]);
  ...
  const handleResetSimulation = () => {
    ...
    // Clear aggressive update interval
    if (aggressiveUpdateIntervalRef.current) {
      clearInterval(aggressiveUpdateIntervalRef.current);
      aggressiveUpdateIntervalRef.current = null;
    }
    ...
    // Clear additional vehicles and tracking
  ...
  const animateAdditionalVehicle = (vehicleId: string) => {
    ...
    const interval = setInterval(() => {
      ...
      if (vehicle.route.length > 1) {
        ...
        // If tracking is enabled and this is the selected vehicle, center map on it (but respect map freeze setting)
        if (isTrackingVehicle && selectedVehicleId === vehicleId && window.googleMap && !isMapViewFreezed) {
          window.googleMap.panTo(newPosition);
        }
        ...
        // Update our UI state with the latest vehicles (only if vehicle is still active)
        const currentVehicle = vehicleManager.getVehicle(vehicleId);
        if (currentVehicle && currentVehicle.status !== 'completed') {
          setAdditionalVehicles([...vehicleManager.getAllVehicles()]);
        }
      }
      
      // If we're done, clean up and mark as completed
      if (progress >= 1) {
        clearInterval(interval);
        console.log(`Animation complete for vehicle ${vehicleId}`);
        
        // Update final state and mark as completed
        vehicleManager.updateVehiclePosition(
          vehicleId,
          vehicle.destination,
          100,
        ...
        // Mark vehicle as completed to prevent further updates
        const completedVehicle = vehicleManager.getVehicle(vehicleId);
        if (completedVehicle) {
          completedVehicle.status = 'completed';
        }
  ...
  const handleSelectVehicle = (vehicleId: string) => {
    ...
    if (vehicle && window.googleMap) {
      // Center map on selected vehicle (user action, so override freeze setting)
  ...
  const handleToggleTracking = () => {
    ...
    const newTrackingState = !isTrackingVehicle;
    setIsTrackingVehicle(newTrackingState);
    
    if (newTrackingState) {
      // If enabling tracking, center on the selected vehicle (user action, so override freeze initially)
      const vehicle = vehicleManager.getVehicle(selectedVehicleId);
      if (vehicle && window.googleMap) {
        window.googleMap.panTo(vehicle.currentPosition);
        window.googleMap.setZoom(16); // Closer zoom for tracking
      }
    }
  }
  ...
  // Stop tracking when selected vehicle changes or is deselected
  useEffect(() => {
    if (!selectedVehicleId && isTrackingVehicle) {
      setIsTrackingVehicle(false);
    }
  }, [selectedVehicleId, isTrackingVehicle]);

  // Add escape key listener to cancel map picker
  useEffect(() => {
    const handleEscapeKey = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && isMapPickerActive) {
        setIsMapPickerActive(null)
      }
    }
  ...
  useEffect(() => {
    // Clear any existing aggressive update interval
    if (aggressiveUpdateIntervalRef.current) {
      clearInterval(aggressiveUpdateIntervalRef.current);
      aggressiveUpdateIntervalRef.current = null;
    }
    ...
    if (isSimulationRunning && !hasReachedDestination && !showReachedMessage) {
      console.log("Setting up aggressive navigation update interval");
      
      // Create a high-frequency update interval
      aggressiveUpdateIntervalRef.current = setInterval(() => {
        // Only update if simulation is still running and destination not reached
        if (routeInfo && routeInfo.steps && routeInfo.steps.length > 0 && 
            isSimulationRunning && !hasReachedDestination && !showReachedMessage) {
          // Calculate current progress
          const progressPercent = animationProgress;
          
          // Stop updates if we've reached 100% - CLEAR THE INTERVAL
          if (progressPercent >= 100) {
            console.log("Navigation updates stopped - journey complete, clearing interval");
            if (aggressiveUpdateIntervalRef.current) {
              clearInterval(aggressiveUpdateIntervalRef.current);
              aggressiveUpdateIntervalRef.current = null;
            }
          ...
          if (newStepIndex !== currentStepIndex) {
            ...
            setRouteInfo({
              ...
              hasReachedDestination: false, // Don't set to true until animation actually completes
              _updateTimestamp: Date.now()
            });
          }
        } else {
  ...
  }, [isSimulationRunning, hasReachedDestination, showReachedMessage]); // Remove animationProgress from dependencies
  ...
  const handleMapPicker = (type: 'start' | 'destination') => {
    setIsMapPickerActive(type)
    
    // Add click listener to the map
    if (window.googleMap) {
      const clickListener = window.googleMap.addListener('click', (event: google.maps.MapMouseEvent) => {
        if (event.latLng) {
          ...
          const tempMarker = new window.google.maps.Marker({
            ...
            icon: {
              ...
              scale: 10,
              fillColor: type === 'start' ? '#10B981' : '#EF4444', // green for start, red for destination
              fillOpacity: 1,
              strokeWeight: 2,
              strokeColor: '#FFFFFF',
            },
            title: type === 'start' ? 'Selected Start Point' : 'Selected Destination',
          ...
          const customEvent = new CustomEvent('map-picker-result', {
            detail: {
              type: type,
              coordinates: { lat, lng }
            }
          ...
          // Clean up listener and reset picker mode
    ...
    <PageLoader>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <ControlPanel
          ...
          onMapPicker={handleMapPicker}
          routeInfo={routeInfo}
        />
        <div className="relative flex-1">
          <MapContainer isDarkMode={isDarkMode}>
            {/* Add cursor styling for map picker mode */}
            {isMapPickerActive && (
              <style jsx global>{`
                .map-container div[role="application"] {
            ...
            {startCoords && destCoords && (
              <RoutePreview 
                ...
                isMapViewFreezed={isMapViewFreezed}
          ...
          {additionalVehicles.length > 0 && (
            <div className={`absolute top-20 left-4 z-20 p-2 rounded-lg shadow-lg ${isDarkMode ? "bg-gray-800 text-white" : "bg-white"}`}>
              ...
              {/* Vehicle tracking button - only show when a vehicle is selected */}
              {selectedVehicleId && (
                <div className="mt-2 pt-2 border-t border-gray-300">
                  <button
                    ...
                    disabled={!selectedVehicleId || additionalVehicles.find(v => v.id === selectedVehicleId)?.status === 'completed'}
                  >
                    {isTrackingVehicle ? '📍 Stop Tracking' : '🎯 Track Vehicle'}
                  </button>
                </div>
              )}
            </div>
          ...
          {isMapPickerActive && (
            <div className={`absolute top-20 left-1/2 transform -translate-x-1/2 z-30 px-4 py-3 rounded-lg shadow-lg ${
              isDarkMode ? 'bg-blue-800 text-white' : 'bg-blue-100 text-blue-800'
            } border-2 border-blue-400 max-w-sm`}>
              <div className="flex items-center gap-2 mb-2">
                <Map className="h-5 w-5" />
                <span className="font-medium">
                  Select {isMapPickerActive === 'start' ? 'Start Point' : 'Destination'}
                </span>
              </div>
              <div className="text-sm space-y-1">
                <div>📍 Click anywhere on the map to set coordinates</div>
                <div className="text-xs opacity-75">
                  Press ESC to cancel • Coordinates will be automatically filled
                </div>
              </div>
              <div className="mt-2 pt-2 border-t border-blue-300 flex justify-between items-center">
                <span className="text-xs font-medium">
                  {isMapPickerActive === 'start' ? '🟢 Start Point' : '🔴 Destination'}
                </span>
                <button 
                  onClick={() => setIsMapPickerActive(null)}
              ...
              <div className={`p-3 font-medium border-b ${isDarkMode ? "border-gray-700 text-white" : "border-gray-200"} sticky top-0 ${isDarkMode ? "bg-gray-800" : "bg-white"} z-10`}>
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    ...
                    {((routeInfo?.steps && routeInfo.steps.length > 0) || additionalVehicles.length > 0) && (
                      <button 
                        className={`text-xs px-2 py-1 rounded ${isDarkMode ? "bg-gray-700 hover:bg-gray-600" : "bg-gray-100 hover:bg-gray-200"}`}
                        onClick={() => setIsTopNavPanelExpanded(!isTopNavPanelExpanded)}
                      >
                        {isTopNavPanelExpanded ? '📋 Compact' : '📋 Expand'}
                      </button>
                    )}
                  </div>
                  <button 
                  ...
                  <div>
                    ...
                    <div className="p-2 space-y-2">
                      {isTopNavPanelExpanded ? (
                        ...
                        // Compact mode - show only current step
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1">
                                ...
                                <p className={`text-xs mt-1 ${
                                  ...
                                  Step {routeInfo.currentStepIndex + 1} of {routeInfo.steps.length}
                                </p>
                              </div>
                            </div>
                          </div>
                        ...
                        {(selectedVehicleId === vehicle.id || additionalVehicles.length === 1) && (
                          <div className="p-2 space-y-2">
                            {isTopNavPanelExpanded ? (
                              ...
                              // Compact mode - show only current step
                              vehicle.currentStepIndex !== undefined && 
                              vehicle.steps[vehicle.currentStepIndex] && (
                                <div className={`p-3 rounded-md border-2 ${
                                  isDarkMode 
                                    ? "bg-blue-900 border-blue-600 text-white" 
                                    : "bg-blue-50 border-blue-300"
                                  ...
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                      <p className={`font-medium ${
                                        ...
                                        {vehicle.steps[vehicle.currentStepIndex].instruction}
                                      </p>
                                      {vehicle.steps[vehicle.currentStepIndex].distance && (
                                        <p className={`text-sm mt-1 ${
                                          isDarkMode ? "text-blue-200" : "text-blue-700"
                                        }`}>
                                          {vehicle.steps[vehicle.currentStepIndex].distance}
                                      ...
                                      <p className={`text-xs mt-1 ${
                                        ...
                                        Step {vehicle.currentStepIndex + 1} of {vehicle.steps.length} • {vehicle.progress.toFixed(0)}% Complete
                                      </p>
                                    </div>
                                  </div>
                                </div>
